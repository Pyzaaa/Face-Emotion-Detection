...
def download_affectnet():
    ...
    api = KaggleApi()
    api.authenticate()
    api.dataset_download_files("fatihkgg/affectnet-yolo-format", path="data", unzip=False)
...
# 3) Trening ViT-B_16 z mixed precision
def train_vit():
    ...
    model.heads.head = nn.Linear(model.heads.head.in_features, 8)  # 8 klas
    ...
    results_dir = "experiments/baseline/vit"
    ...
    for epoch in range(EPOCHS):
        ...
        if val_acc > best_acc:
            best_acc = val_acc
            torch.save(model.state_dict(), "models/vit_b16_best.pth")
            print("Zapisano nowy najlepszy model")

    print("\nTrening ViT zakończony. Najlepsza dokładność:", best_acc)

    with open(summary_path, "w") as f:
        f.write("ViT baseline\n")
