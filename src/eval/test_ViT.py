...
if __name__ == "__main__":
    ...
    for imgs, labels, _ in tqdm(test_loader, desc="Evaluating ViT-B/16"):
        imgs = imgs.to(device)
    ...
    print(f"\nDokładność ViT-B/16 na zbiorze testowym: {acc:.4f}")
    print(f"Macro precision: {p_macro:.4f}")
    print(f"Macro recall:    {r_macro:.4f}")
    print(f"Macro F1:        {f1_macro:.4f}")

    with open(metrics_path, "w") as f:
        f.write("ViT-B/16 baseline - test metrics\n")
    ...
    plt.title("Confusion matrix - ViT-B/16 baseline")
    plt.colorbar()
