...
def train_vit_cwgan():
    # oryginalny AffectNet dla valid/test
    ...
    for epoch in range(EPOCHS):
        ...
        if val_acc > best_acc:
            best_acc = val_acc
            torch.save(model.state_dict(), "models/vit_b16_cwgan_gp_best.pth")
            print("Zapisano nowy najlepszy model (cwgan_gp).")

    print("\nTrening ViT-B/16 (cwgan_gp) zakończony. Najlepsza dokładność:", best_acc)

    with open(summary_path, "w") as f:
        f.write("ViT-B/16 cwgan_gp train on balanced dataset\n")
