...
def train_mobilenet_v3_cwgan():
    # upewnij się, że oryginalny AffectNet jest pobrany (dla valid/test)
    yolo_path = download_affectnet()

    # train - zbalansowany zbiór z cWGAN-GP
    aug_yolo_path = "data/affectnet_cwgan_aug/YOLO_format"
    train_img_dir = os.path.join(aug_yolo_path, "train", "images")
    train_lbl_dir = os.path.join(aug_yolo_path, "train", "labels")

    # validation - oryginalny valid z AffectNet
    ...
    print(f"[INFO] Rozmiar train (cwgan_aug): {len(train_dataset)}")
    print(f"[INFO] Rozmiar valid (raw):      {len(val_dataset)}")
    ...
    # osobny katalog na eksperymenty cwgan_gp
    ...
    for epoch in range(EPOCHS):
        ...
        if val_acc > best_acc:
            best_acc = val_acc
            torch.save(model.state_dict(), "models/mobilenet_v3_cwgan_gp_best.pth")
            print("Zapisano nowy najlepszy model (cwgan_gp).")

    print("\nTrening MobileNetV3 (cwgan_gp) zakończony. Najlepsza dokładność:", best_acc)

    with open(summary_path, "w") as f:
        f.write("MobileNetV3 cwgan_gp train on balanced dataset\n")
